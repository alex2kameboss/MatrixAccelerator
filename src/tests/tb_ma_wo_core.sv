...
task broadcast_l_operation_test;
    input register      rr      ;
    input xif_t         rr_prf_x;
    input xif_t         rr_prf_y;
    input register      r1      ;
...
begin
    ...
    int i_rs1, i_rd;
    int r, c;
    $display("Broadcast left operation test");
    define_register_one_step(
        .r    ( rr      ),
        .w    ( w       ),
        .h    ( h       ),
...
task broadcast_r_operation_test;
    input register      rr      ;
    input xif_t         rr_prf_x;
    input xif_t         rr_prf_y;
    input register      r1      ;
...
begin
    ...
    int i_rs1, i_rd;
    int r, c;
    $display("Broadcast right operation test");
    define_register_one_step(
        .r    ( rr      ),
        .w    ( w       ),
        .h    ( h       ),
...
task vector_vector_operation_test_small;
    input register      rr      ;
    input xif_t         rr_prf_x;
    input xif_t         rr_prf_y;
    input register      r1      ;
...
begin
    int bytes;
    int i, r, c;
    $display("Vector-Vector small size operation test");
    define_register_one_step(
        .r    ( rr      ),
        .w    ( w       ),
        .h    ( h       ),
    ...
    for ( r = 0; r < h; r = r + 1 ) begin
        for ( c = 0; c < w; c = c + 1 ) begin
            i = (r * roundUp(w, PRF_N_LANES) + c) * bytes;
            if ( dt == INT32 | dt == UINT32 ) begin
            assert({i_sim_mem.i_sim_mem.mem[rr_addr + i + 3], i_sim_mem.i_sim_mem.mem[rr_addr + i + 2], i_sim_mem.i_sim_mem.mem[rr_addr + i + 1], i_sim_mem.i_sim_mem.mem[rr_addr + i + 0]} == 
                alu({i_sim_mem.i_sim_mem.mem[r1_addr + i + 3], i_sim_mem.i_sim_mem.mem[r1_addr + i + 2], i_sim_mem.i_sim_mem.mem[r1_addr + i + 1], i_sim_mem.i_sim_mem.mem[r1_addr + i + 0]}, 
                    {i_sim_mem.i_sim_mem.mem[r2_addr + i + 3], i_sim_mem.i_sim_mem.mem[r2_addr + i + 2], i_sim_mem.i_sim_mem.mem[r2_addr + i + 1], i_sim_mem.i_sim_mem.mem[r2_addr + i + 0]}, o)[31:0]) else
...
task convolution_operation_test_small;
    input register      rr      ;
    input xif_t         rr_prf_x;
    input xif_t         rr_prf_y;
    input dtype_t       rr_dt   ;
...
begin
    ...
    for ( i = 0; i < h - h_k + 1; i = i + 1 )
        for ( j = 0; j < w - w_k + 1; j = j +1 ) begin
            ...
            for ( ii = 0; ii < h_k; ii = ii + 1 )
                for ( jj = 0; jj < w_k; jj = jj + 1 )
                    el = el + data_concat(r1_addr + ( (i + ii) * roundUp(w, PRF_N_LANES) + j + jj ) * bytes_r1, r1_dt) * data_concat(r2_addr + ( ii * kernel_w + jj ) * bytes_r2, r2_dt);
            if ( rr_dt == INT32 | rr_dt == UINT32 ) begin
                assert(el == data_concat(rr_addr + ( i * roundUp(w, PRF_N_LANES) + j ) * bytes_rr, rr_dt)) else
                $error("i: %d, j: %d, rd_expected: %d, rd_computed: %d", i, j, el, data_concat(rr_addr + ( i * roundUp(w, PRF_N_LANES) + j ) * bytes_rr, rr_dt));
            end
            else if ( rr_dt == INT16 | rr_dt == UINT16 ) begin
                assert(el[15 : 0] == data_concat(rr_addr + ( i * roundUp(w, PRF_N_LANES) + j ) * bytes_rr, rr_dt)[15 : 0]) else
                $error("i: %d, j: %d, rd_expected: %d, rd_computed: %d", i, j, el, data_concat(rr_addr + ( i * roundUp(w, PRF_N_LANES) + j ) * bytes_rr, rr_dt));
            end
            else if ( rr_dt == INT8 | rr_dt == UINT8 ) begin
                assert(el[7 : 0] == data_concat(rr_addr + ( i * roundUp(w, PRF_N_LANES) + j ) * bytes_rr, rr_dt)[7 : 0]) else
                $error("i: %d, j: %d, rd_expected: %d, rd_computed: %d", i, j, el, data_concat(rr_addr + ( i * roundUp(w, PRF_N_LANES) + j ) * bytes_rr, rr_dt));
            end
        end
...
//
    ...
    // ------- test small vector vector operations -------
    vector_vector_operation_test_small(
        .rr      ( 'd2      ),
        .rr_prf_x( 'd0      ),
        .rr_prf_y( 'd64     ),
        .r1      ( 'd0      ),
    ...
    convolution_operation_test_small(
        .rr      ( 'd2      ),
        .rr_prf_x( 'd64     ),
        .rr_prf_y( 'd64     ),
        .rr_dt   ( INT32    ),
