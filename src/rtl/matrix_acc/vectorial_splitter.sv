...
logic [7 : 0] op_in_byte [NUMBER_OF_ALU - 1 : 0][IN_BYTES - 1 : 0];
...
assign op_in_byte[i_split][j_split] = op_in[i_split * OUT_DATA_WIDTH + (j_split + 1) * 8 - 1 -: 8];
        end
    end
endgenerate
...
always_ff @(posedge clk, negedge rst_n)
    if ( ~rst_n ) begin
        ...
        if ( dtype == ma_pkg::INT32 | dtype == ma_pkg::UINT32 ) begin
            ...
            op_out[i] <= {{(OUT_DATA_WIDTH - 16){is_signed & op_in_byte[i][2 * cnt + 1][7]}}, op_in_byte[i][2 * cnt + 1], op_in_byte[i][2 * cnt + 0]};
        end else begin
            op_out[i] <= {{(OUT_DATA_WIDTH - 8){is_signed & op_in_byte[i][cnt][7]}}, op_in_byte[i][cnt]};
        end
    end
end
endgenerate
